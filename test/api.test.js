...
describe('API', { concurrency: false }, () => {
  ...
  test('TEAM-03 creates a team and derives its slug when omitted', async () => {
    const response = await request('POST', '/api/teams', {
      name: 'U13 Girls Team', description: 'Junior squad',
    });
    ...
    assert.deepEqual(stored, { name: 'U13 Girls Team', slug: 'u13-girls-team', description: 'Junior squad' });
  });
  ...
  test('PERSON-02 creates a team member with the default participant role', async () => {
    const response = await request('POST', `/api/teams/${team.slug}/persons`, { name: 'Bob Example' });
    assert.equal(response.status, 201);
  ...
  test('PERSON-04 creates a person with explicit roles', async () => {
    const response = await request('POST', `/api/teams/${team.slug}/persons`, { name: 'Coach Example', roles: ['participant', 'coach'] });
    assert.equal(response.status, 201);
  ...
  test('PERSON-05 rejects an empty role list', async () => {
    const response = await request('POST', `/api/teams/${team.slug}/persons`, { name: 'No Role', roles: [] });
    assert.equal(response.status, 400);
  ...
  test('PERSON-07 adds an existing person to the team with the supplied roles', async () => {
    ...
    const response = await request('POST', `/api/teams/${team.slug}/persons/${existing.id}`, { roles: ['coach'] });
    assert.equal(response.status, 200);
  ...
  test('PERSON-08 rejects adding an existing person who is already a team member', async () => {
    const response = await request('POST', `/api/teams/${team.slug}/persons/${person.id}`, { roles: ['coach'] });
    assert.equal(response.status, 409);
  ...
  test('PERSON-09 allows a team manager to update member roles', async () => {
    setAccount({ isSiteAdmin: false, teamRoles: [{ teamId: team.id, role: 'team_manager' }] });
    const response = await request('PUT', `/api/teams/${team.slug}/persons/${person.id}/roles`, { roles: ['coach'] });
    assert.equal(response.status, 200);
  ...
  test('PERSON-10 allows a team manager to remove a member', async () => {
    setAccount({ isSiteAdmin: false, teamRoles: [{ teamId: team.id, role: 'team_manager' }] });
    const response = await request('DELETE', `/api/teams/${team.slug}/persons/${person.id}`);
    assert.equal(response.status, 200);
  ...
  test('PERSON-11 rejects person management for a team member without people permission', async () => {
    setAccount({ isSiteAdmin: false, teamRoles: [{ teamId: team.id, role: 'team_member' }] });
    const response = await request('POST', `/api/teams/${team.slug}/persons/${person.id}`, { roles: ['coach'] });
    assert.equal(response.status, 403);
    assert.deepEqual(response.body, { error: 'insufficient team permissions' });
  });

  test('PERSON-12 allows a team manager to update a person name', async () => {
    setAccount({ isSiteAdmin: false, teamRoles: [{ teamId: team.id, role: 'team_manager' }] });
    const response = await request('PUT', `/api/teams/${team.slug}/persons/${person.id}`, { name: 'Renamed Person' });
    assert.equal(response.status, 200);
    const member = response.body.find((item) => item.id === person.id);
    assert.equal(member.name, 'Renamed Person');
    assert.equal(getDb().prepare('SELECT name FROM persons WHERE id = ?').get(person.id).name, 'Renamed Person');
  });
  ...
  test('EVENT-02 creates an event with its supplied fields', async () => {
    const response = await request('POST', `/api/teams/${team.slug}/events`, {
      subject: 'League Match', date: '2026-09-05', startTime: '10:00', endTime: '12:00', location: 'Main Stadium', description: 'Home match',
    });
    assert.equal(response.status, 201);
  ...
  test('EVENT-03 rejects a category belonging to another team', async () => {
    ...
    getDb().prepare('INSERT INTO categories (id, team_id, name, color) VALUES (?, ?, ?, ?)').run(categoryId, otherTeam.id, 'Other Category', '#123456');
    const response = await request('POST', `/api/teams/${team.slug}/events`, { categoryId, subject: 'Invalid Category Event', date: '2026-09-05' });
    assert.equal(response.status, 400);
  ...
  test('EVENT-04 updates an event', async () => {
    const response = await request('PUT', `/api/teams/${team.slug}/events/${event.id}`, {
      subject: 'Updated Training', date: '2026-09-03', startTime: '20:00', endTime: '21:30', location: 'New Hall', description: 'Updated description',
    });
    assert.equal(response.status, 200);
    const updated = response.body.find(e => e.id === event.id);
    assert.deepEqual(updated, { id: event.id, categoryId: null, subject: 'Updated Training', date: '2026-09-03', startTime: '20:00', endTime: '21:30', location: 'New Hall', description: 'Updated description' });
  });
  ...
  test('ATT-01 sets yes, maybe and no attendance statuses', async () => {
    for (const status of ['yes', 'maybe', 'no']) {
      ...
      assert.equal(response.status, 200);
      const stored = getDb().prepare('SELECT status, note FROM attendance WHERE person_id = ? AND event_id = ?').get(person.id, event.id);
      assert.deepEqual(stored, { status, note: '' });
    }
  });

  test('ATT-02 updates existing attendance instead of creating duplicates', async () => {
    await request('PUT', `/api/teams/${team.slug}/attendance/${person.id}/${event.id}`, { status: 'yes', note: 'Arrives late' });
    await request('PUT', `/api/teams/${team.slug}/attendance/${person.id}/${event.id}`, { status: 'maybe', note: 'May arrive late' });
    const rows = getDb().prepare('SELECT status, note FROM attendance WHERE person_id = ? AND event_id = ?').all(person.id, event.id);
  ...
  test('ATT-04 returns the team attendance matrix', async () => {
    await request('PUT', `/api/teams/${team.slug}/attendance/${person.id}/${event.id}`, { status: 'yes', note: 'On time' });
    const response = await request('GET', `/api/teams/${team.slug}/attendance`);
    assert.equal(response.status, 200);
    assert.deepEqual(response.body, { [person.id]: { [event.id]: { status: 'yes', note: 'On time' } } });
  });
  ...
  test('ATT-05 rejects attendance for a person who is not a team member', async () => {
    ...
    const response = await request('PUT', `/api/teams/${team.slug}/attendance/${outsider.id}/${event.id}`, { status: 'yes' });
  ...
  test('ATT-06 rejects attendance for an event belonging to another team', async () => {
    ...
    const response = await request('PUT', `/api/teams/${team.slug}/attendance/${person.id}/${otherEvent.id}`, { status: 'yes' });
    assert.equal(response.status, 404);
    assert.deepEqual(response.body, { error: 'person or event not found in team' });
  });
});
