...
const MESSAGES = {
  en: {
    ...
    syncHint:
      'Paste this link into your calendar app\u2019s "subscribe by URL" option to keep it in sync automatically.',
    ...
    confirmRemoveAllRoles:
      "Removing all roles will delete this person. Continue?",
    ...
    bulkAttendanceHint:
      "Set attendance for multiple events at once. Nothing changes until you choose an action and confirm it.",
    ...
    personCalendarHint:
      "Select a person to get a calendar containing events where that person is going or is assigned as staff.",
    ...
    removeAllAttendanceConfirm:
      "Remove all attendance records for this Team? This cannot be undone.",
    removeAllEventsConfirm:
      "Remove all events for this Team? This cannot be undone.",
  ...
  "nl-BE": {
    ...
    syncHint:
      'Plak deze link bij "abonneren via URL" in je kalender-app om automatisch synchroon te blijven.',
    ...
    pickPlayer:
      "Kies een speler en geef aan bij welke events die aanwezig zal zijn.",
    player: "Speler",
    ...
    overviewSubtitle:
      "Overzicht van alle events met aanwezigheid en staff-toewijzingen.",
    ...
    staffAssignmentsSubtitle:
      "Wijs staff toe aan events op basis van de vereiste rollen.",
    noneAssigned: "— Geen —",
    noEligibleStaff: "Geen staff met deze rol beschikbaar.",
    adminSubtitle: "Beheer personen, categorieën en events.",
    persons: "Personen",
    peopleHint:
      "Personen met rollen: deelnemer, coach, assistent-coach of trainer.",
    ...
    confirmRemoveAllRoles:
      "Als je alle rollen verwijdert, wordt deze persoon verwijderd. Doorgaan?",
    ...
    bulkAttendanceHint:
      "Stel de aanwezigheid voor meerdere events tegelijk in. Er verandert niets totdat je een actie kiest en bevestigt.",
    ...
    personCalendarHint:
      "Selecteer een persoon om een kalender te krijgen met events waarvoor die persoon aanwezig is of als staff is toegewezen.",
    ...
    couldNotUpdateAttendance:
      "Aanwezigheid kon niet worden opgeslagen. Probeer het opnieuw.",
    staff: "Staff",
    unknown: "Onbekend",
    peopleNav: "Personen",
    ...
    teams: "Teams",
    teamsSubtitle:
      "Beheer teams en teamoverstijgend lidmaatschap. Teamspecifieke rollen worden binnen elk team beheerd.",
    ...
    deleteTeamConfirm:
      "Dit team en al zijn events verwijderen? Personen die tot geen enkel ander team behoren worden ook verwijderd.",
    removeAllAttendanceConfirm:
      "Alle aanwezigheidsgegevens voor dit team verwijderen? Dit kan niet ongedaan worden gemaakt.",
    removeAllEventsConfirm:
      "Alle events voor dit team verwijderen? Dit kan niet ongedaan worden gemaakt.",
...
function formatDate(iso) {
  ...
  return d.toLocaleDateString(
    currentLanguage === "nl-BE" ? "nl-BE" : undefined,
...
function categoryBadge(categories, categoryId) {
  ...
  if (!cat)
    return `<span class="badge" style="background:#0000000f;color:var(--ink-soft)">${t("uncategorized")}</span>`;
...
const ALL_ROLES = [
  { id: PARTICIPANT_ROLE, label: t("participant") },
  ...STAFF_ROLES,
];
...
function staffRoleLabels(ids) {
  return STAFF_ROLES.filter((r) => (ids || []).includes(r.id)).map(
    (r) => r.label,
  );
}
...
function filterEvents(events, filters) {
  ...
  return events.filter((ev) => {
    if (
      ...
      !filters.categoryIds.has(ev.categoryId)
    )
      return false;
