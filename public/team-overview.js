...
function handleOverviewClick(event) {
  ...
  if (target.matches(".attendance-person")) {
    ...
    toggleStaffAssignment(target.dataset.staffPerson, target.dataset.staffEvent, target.dataset.staffRole);
  }
}
function bindEventHandlers() {
  const box = document.getElementById("events");
  box.onclick = handleOverviewClick;
...
async function saveNote(button) {
  ...
  const personId = editor.dataset.editorPerson, eventId = editor.dataset.editorEvent;
