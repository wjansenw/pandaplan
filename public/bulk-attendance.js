...
(function () {
  ...
  function ensureBlock() {
    ...
    const toggleBlock = () => {
      ...
      toggle.querySelector(".bulk-expand-hint").lastChild.textContent =
  ...
  async function apply(status) {
    ...
    try {
      ...
      events.forEach((event) => {
        ...
        state.attendance[participantId][event.id] = {
          status,
          note: old.note || "",
        };
      });
      ...
      alert(
        t("couldNotSaveAttendance", {
          error: error.message || t("tryAgain"),
        }),
      );
    }
  }
