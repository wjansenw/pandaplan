...
function buildPersonCalendarIcs(personId) {
  ...
  const events = db.prepare(`
    SELECT DISTINCT e.id, e.team_id AS teamId, e.category_id AS categoryId,
      e.date, e.start_time AS startTime, e.end_time AS endTime,
      e.location, e.description
    FROM events e
    JOIN team_memberships tm ON tm.team_id = e.team_id AND tm.person_id = ?
    ...
    WHERE a.status = 'yes' OR sa.person_id IS NOT NULL
